...
/*串口接收函数（将要发送的数据压入缓冲区）
	...
	解释:如果在指定等待时间内接收不到足够数据（接收缓冲区里没这么多数据）
				就接收尽量多的数据
*/
uint16_t Read_Uart1( uint8_t *data, uint16_t length, double Rc_waitTime, double Sync_waitTime )//maxi:串口接收函数为什么会有输入参数？
	{   //maxi:弄清楚串口接收到数据到底把数据放在哪里了。
	...
	{	
		uint16_t rc_length = xStreamBufferReceive( RxStreamBuffer , data , length , Rc_waitTicks );  //maxi:这应该是最关键的函数。

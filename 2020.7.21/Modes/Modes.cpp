		...
		if( msg_available )
		{
			switch( msg.cmd )
			{
				...
				{	//do set mode
					...
					{	//mavlink定义模式
						px4_custom_mode mav_mode;
						mav_mode.data = msg.params[1];
						if( mav_mode.main_mode==PX4_CUSTOM_MAIN_MODE_AUTO && mav_mode.sub_mode==PX4_CUSTOM_SUB_MODE_AUTO_MISSION )
							changeMode( 32, 0, MAV_MODE_FLAG_AUTO_ENABLED, 0 );
					}
					//模式执行完毕返回本模式
					setLedMode(LEDMode_Normal1);
					last_pre_enter_mode = 0;
